  ...
  return (
    ...
    >
      <span
        ...
        className={`h-2 w-2 shrink-0 rounded-full ${DOT[task.state]}`}
      />
      ...
      <div className="min-w-0 flex-1">
        ...
        {hasJump && !editing && (
          <div className="flex items-center gap-1 truncate text-[10px] text-white/35">
            {task.jump_type === "scene" ? (
              <span
                className="shrink-0 rounded bg-white/[0.08] px-1 text-white/45"
                title={`Scene — ${sceneCount} workspace${sceneCount === 1 ? "" : "s"} across monitors`}
              >
                ⧉ {sceneCount}
              </span>
            ) : task.jump_type === "workspace" ? (
              <span
                className="shrink-0 rounded bg-white/[0.08] px-1 text-white/45"
                title="Single workspace (focused monitor only)"
              >
                ▭
              </span>
            ) : null}
      ...
      <div className="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
        {!done && (
          <>
            <IconBtn title="Defer — send to the bottom" onClick={() => onDefer(task)}>
              ↓
            </IconBtn>
            <IconBtn
